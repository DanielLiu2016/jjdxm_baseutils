...
public class SDKHelper {
    ...
    private void InitTIMSDK() {
        //设置log级别
        TIMManager.getInstance().setLogPrintEanble(BaseApplication.getApplication().getLogConsole());
        int loglevel = BaseApplication.getApplication().getLogLevel();
        ...
        if (BaseApplication.getApplication().getTestEnvSetting()) {
            Log.e(TAG, "set test env");
            TIMManager.getInstance().setEnv(1);
        }
        TIMManager.getInstance().init(context, QQIMParamsConfig.SDK_APPID, String.valueOf(QQIMParamsConfig.ACCOUNT_TYPE));
        SetConnCallBack();
