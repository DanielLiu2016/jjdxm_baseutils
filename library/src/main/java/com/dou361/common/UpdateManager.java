...
import com.dou361.bean.Update;
import com.dou361.utils.CheckCodeUitls;
...
import com.jingwang.eluxue_online.protocol.CheckAppVersionCodeProtocol;
...
public class UpdateManager {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case DOWN_NOSDCARD:
                    downloadDialog.dismiss();
                    UIUtils.showToastShort("无法下载安装文件，请检查SD卡是否挂载");
                    break;
            }
        }
    ...
    public void checkAppUpdate(Context context, final boolean isShowMsg) {
        ...
        new Thread() {
            public void run() {
                CheckAppVersionCodeProtocol mCheckAppVersionCodeProtocol = new CheckAppVersionCodeProtocol(SystemUtils.getVersionCode() + "");
                Update update = mCheckAppVersionCodeProtocol.load(1);
                int code = mCheckAppVersionCodeProtocol.getResponeCode();
    ...
    private Runnable mdownApkRunnable = new Runnable() {
        ...
        public void run() {
            try {
                String apkName = "app_" + mUpdate.getVersionName() + ".apk";
                String tmpApk = "app_" + mUpdate.getVersionName() + ".tmp";
                //判断是否挂载了SD卡
                String storageState = Environment.getExternalStorageState();
                if (storageState.equals(Environment.MEDIA_MOUNTED)) {
                    savePath = FileUtils.getDownloadDir();
